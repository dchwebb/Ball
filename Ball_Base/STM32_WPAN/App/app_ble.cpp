...
typedef struct _tSecurityParams {
	...
	uint8_t mitm_mode;				// Authentication:  Man In the Middle protection required
	uint8_t bonding_mode;
	uint8_t Use_Fixed_Pin;			// use a fixed pin during the pairing process (or passkey has to be requested to the application during the pairing process)
	...
	// 0x02 : host need not send the clave security request but it has to wait for paiirng to complete before doing any other processing
...
typedef struct _tBLEProfileGlobalContext {
	...
	uint8_t advtServUUID[100];			// the UUID list to be used while advertising
} BleGlobalContext_t;
			...
			if (event_type == ADV_IND)
			{
				...
				while(k < event_data_size)
				{
					...
					case AD_TYPE_SERVICE_DATA: /* service data 16 bits */
						...
						break;
					default:
						if (adtype == AD_TYPE_16_BIT_SERV_UUID_CMPLT_LIST) {
							if (adv_report_data[k] == 3 && (*(uint16_t*)&adv_report_data[k + 2]) == 0x1812) {
								APP_DBG_MSG("-- SERVER DETECTED -- HID Device\n");
								BleApplicationContext.DeviceServerFound = 0x01;
								SERVER_REMOTE_BDADDR[0] = le_advertising_event->Advertising_Report[0].Address[0];
								SERVER_REMOTE_BDADDR[1] = le_advertising_event->Advertising_Report[0].Address[1];
								SERVER_REMOTE_BDADDR[2] = le_advertising_event->Advertising_Report[0].Address[2];
