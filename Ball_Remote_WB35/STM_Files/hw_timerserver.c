...
static uint32_t ReadRtcSsrValue(void)
{
	uint32_t first_read = (uint32_t)(READ_BIT(RTC->SSR, RTC_SSR_SS));
	uint32_t second_read = (uint32_t)(READ_BIT(RTC->SSR, RTC_SSR_SS));
...
static void LinkTimerAfter(uint8_t TimerID, uint8_t RefTimerID)
{
	 uint8_t next_id = aTimerContext[RefTimerID].NextID;
...
static uint16_t ReturnTimeElapsed(void)
{
	...
	if (SSRValueOnLastSetup != SSR_FORBIDDEN_VALUE)	{
		...
		// At this stage, ReturnValue holds the number of ticks counted by SSR Need to translate in number of ticks counted by the Wakeuptimer
...
static void RestartWakeupCounter(uint16_t Value)
{
	...
	if (Value == 0)	{
		...
		HAL_NVIC_ClearPendingIRQ(RTC_WKUP_IRQn);					//  Clear pending bit in NVIC
...
void HW_TS_Init(HW_TS_InitMode_t TimerInitMode, RTC_HandleTypeDef *phrtc)
{
	...
	HAL_NVIC_SetPriority(RTC_WKUP_IRQn, CFG_HW_TS_NVIC_RTC_WAKEUP_IT_PREEMPTPRIO, CFG_HW_TS_NVIC_RTC_WAKEUP_IT_SUBPRIO);   /**<  Set NVIC priority */
...
void HW_TS_Start(uint8_t timer_id, uint32_t timeout_ticks)
{
	...
	uint16_t time_elapsed =  linkTimer(timer_id);

	uint8_t localcurrentrunningtimerid = CurrentRunningTimerID;
